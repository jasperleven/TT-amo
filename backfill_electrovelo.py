...
# Байеры, работающие с BC Насти (пиксель "Электровелосипед общий") — сверено
# по фактически активным строкам в tt_links_2.xlsx на 27.08.2026 (тот же день,
# что и в текущей проверке). VLD и KRL за этот день не показывали активности
# (VLD вообще не встречается, KRL — только 0-лидовые технические клики), но
# оставляем список открытым: пересчитывай под нужную дату при смене периода.
NASTYA_BC_BUYER_TAGS = ["VAD", "ART", "BNS"]
# ─────────────────────────────────────────────────────────────────────
